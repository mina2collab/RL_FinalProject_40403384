...
OUTPUT_PATH = (
    ...
    / "results"
    / "figures"
    / "q_learning_training_curves.png"
)

INPUT_FILES = {
    "Linear epsilon decay": (
        RAW_DATA_DIR
        / "q_learning_linear_training.csv"
    ),
    "Exponential epsilon decay": (
        RAW_DATA_DIR
        / "q_learning_exponential_training.csv"
    ),
}
...
def load_training_history(
    ...
    """Load episode results from a CSV file."""

    if not file_path.exists():
        raise FileNotFoundError(
            f"Training file was not found:\n"
            f"{file_path}\n\n"
            "Run experiments/run_q_learning.py first."
        )
...
def generate_figure() -> Path:
    """Generate Q-Learning training curves."""
